...
三張台灣 xlsx 只有料號沒有支數，而支數會直接影響「共 N 支 $X」的報價與能不能湊成一套。優先從
NGK 台灣官網查（`api_plug_counts()`）；官網查不到的車逐台查證後寫在 `TW_TABLE_CYLINDERS`，
每一筆都註明來源是 NGK 官網的同車別名還是原廠公布的引擎規格。**都查不到就維持 `null`**，前端顯示
「請依引擎缸數選購數量」只報單支價，不印一個猜出來的「此車需 1 支」。

這裡抓到一台不是 1 支的：**KYMCO CV3 是 550.4cc 並列雙缸（AK550 那顆引擎），要 2 支**，
其餘 26 台查證後都是單缸。若當初預設 1 支，CV3 車主會少買一半。

## 目前涵蓋範圍（2026-08-06）

- 19 廠牌 / **757 車款** / 822 個車款年份組合（改版前：9 廠牌 202 車款）
- 原廠型錄 618 車款 ／ 台灣對照表 81 ／ 台灣官網 58
- **821 個組合可直接導購**，1 個現貨不足
...
**只顯示台灣市場有的廠牌**（Jerry 2026-08-06 指定）—— 日本型錄收的是全世界的車，包含台灣從來沒進過、
甚至已經倒掉的廠牌（Husaberg 2013 併入 Husqvarna、Vertemati 與 Malaguti 早就沒了）。名單寫在
`build_data.TW_MARKET_BRANDS`，用**白名單**不用黑名單 —— 以後型錄新增廠牌應該要有人判斷過才上架，
而不是因為沒人記得排除就自己出現。有台灣總代理但目前還沒有 MotoDX 車款的（DUCATI、BMW、哈雷…）
也寫在名單裡，NGK 哪天出了就會自己冒出來。目前隱藏 9 廠牌 / 42 車款（Jerry 2026-08-06 指名把 Italjet、TM Racing、Adiva、GasGas、Beta 加回，那五個台灣有）。

**只顯示原廠／標準與 MotoDX 兩欄**（Jerry 2026-08-06 指定）—— 店裡沒賣銥合金與白金，列出來等於幫
買不到的產品打廣告。這兩欄連 `data.json` 都不輸出，順便讓每個訪客要下載的檔案小 43KB。

**鐵律：不做替代料號推測。** 不拿熱值相近的塞硬湊 —— 熱值/牙距裝錯會傷引擎。

### 已知待確認

兩項原本待確認的，2026-08-06 查證後已定案：

- `NEW MANY 125(六期)`：台灣對照表寫 `CPR7EDX-9S`、NGK 台灣官網寫 `CR8EDX-S` ——
  **官網對，對照表錯**。官網是按引擎代號回答的：125 是 SE24CD/CE/CF(SE24A) → 原廠 CR8E，
  110 是 SE22CA/CB/CC(SE22) → 原廠 CPR7EA-9，兩顆不同引擎；xlsx 把 125 六期排在
  NEW MANY 110 那一組底下才會沿用到 110 的料號。另外兩邊都同意 125 七期是 CR8E，
  六期 CPR7EA-9 →七期 CR8E 等於逆著走，KYMCO 其他車都是 CR → CPR 不會回頭。
  修正寫在 `TW_TABLE_ROW_FIXES`（原廠與 DX 一起改，只改一半會印出不成對的規格表）。
- 支數：27 台已全部查證補齊，見上節。
